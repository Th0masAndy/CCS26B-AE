...
"$ROOT_DIR/scripts/preflight.sh" --build
echo
...
build_blake3()
{
    ...
    cmake -S "$source_dir/c" -B "$source_dir/c/build" -DCMAKE_INSTALL_PREFIX="$INSTALL_DIR"
...
cmake -S "$ROOT_DIR/code" -B "$BUILD_DIR" -DCMAKE_BUILD_TYPE=Release
