...
matching-boundary datasets and reference answers.
...
\begin{compactitem}
\item[(C1)] The implemented one-sided FPSI protocols recover exact fuzzy
intersections on the supplied matching-boundary cases (E1).
...
dataset/mode combinations across 16 protocol modes. Cases include zero/full
...
in both \path{summary.md} and \path{paper-comparison.md}, and three
runtime SVGs in \path{paper-plots/}. The terminal reports
\texttt{Claim 2 full complete}. A 16\,GiB alternative,
\texttt{bash} \path{claims/claim2/run.sh} \texttt{--light}, retains all dimensions and
thresholds only at $n=2^{12}$ (30 configurations); it does not validate the
other set sizes.
...
in each summary/comparison Markdown table and three runtime SVGs, one per metric.
